...
function updateAdminDetails() {
  ...
  if(!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    document.getElementById('username_error').innerHTML = "ne doit pas contenir d'espaces!";
    username.focus();
  }
  else {
    ...
    if(validatePassword(password)) {
      ...
      document.getElementById('admin_acknowledgement').innerHTML = "<span class='text-danger'> Password Invalide!</span>";
    return false;
  }
}
...
function checkAdminPassword(password, error) {
  ...
  if(validatePassword(password)) {
    ...
    document.getElementById(error).innerHTML = "Erreur Password!!!";
  return false;
}
...
function changePassword() {
  ...
  if(!checkAdminPassword(old_password.value, 'old_password_error'))
    ...
    document.getElementById('password_error').style.display = "block";
    document.getElementById('password_error').innerHTML = "doit contenir plus de 6 caracteres!";
    password.focus();
  }
  else if(password.value != confirm_password.value) {
    document.getElementById('password_error').style.display = "none";
    document.getElementById('confirm_password_error').style.display = "block";
    document.getElementById('confirm_password_error').innerHTML = "password ne correspondent pas!";
