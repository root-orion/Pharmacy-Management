function notNull(text, error) {
  ...
  if(text < 0) {
    ...
    result.innerHTML = "Veuillez remplir le champ svp!";
    return false;
  }
...
function validateName(name, error) {
  ...
  result.style.display = "block";
  if(name.trim() == "") {
    result.innerHTML = "Ne peut etre vide!!!";
    return false;
  }
  result.innerHTML = "doit contenir seulement des lettres!";
...
function validateContactNumber(contact_number, error) {
  ...
  if(contact_number.length != 10) {
    result.innerHTML = "Doit contenir 10 chiffres!";
    return false;
  }
  else
    result.style.display = "none";
...
function validateAddress(address, error) {
  ...
  if(address.trim().length < 10) {
    result.innerHTML = "Entrer une  addresse precise svp!";
    return false;
  }
  else
    result.style.display = "none";
...
function checkExpiry(date, error) {
  ...
  if(date.trim() == "" || date.trim().length != 5 || date[2] != "/")
    result.innerHTML = "enter la  date au format mm/yy !";
    ...
    result.innerHTML = "Medicament perime!";
    return -1;
  }
  else {
    result.style.display = "none";
...
function checkQuantity(quantity, error) {
  ...
  if(quantity < 0 || !Number.isInteger(parseFloat(quantity)))
    result.innerHTML = "Quantite invalide!";
  else {
    result.style.display = "none";
    return true;
  }
...
function checkDate(date, error) {
  ...
  if(date == "")
    ...
    result.innerHTML = "Entrer une date valide ou future!";
  else {
    result.style.display = "none";
    return true;
  }
