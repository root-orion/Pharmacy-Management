...
function removeRow(row_id) {
  if(rows == 1)
    alert("Ligne unique! Impossible de supprimer");
  else {
...
function checkAvailableQuantity(value, id) {
  ...
  if(Number.parseInt(xhttp.responseText) == 0) {
    ...
    document.getElementById("medicine_name_error_" + id).innerHTML = "En rupture de stock!";
    ...
    document.getElementById("quantity_error_" + id).innerHTML = "Seulement " + xhttp.responseText + " en stock!";
    return -2;
  }
  return 999;
}
...
function addInvoice() {
  ...
  if(!notNull(customers_name.value, "customer_name_error"))
    ...
    document.getElementById("customer_name_error").innerHTML = "Client inexistant!";
    customers_name.focus();
  }
  else if(isInvoiceExist(invoice_number.value) == "true")
    document.getElementById("invoice_acknowledgement").innerHTML = "Facture deja enegistree!";
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      if(!notNull(medicine_name.value, medicine_name_error.getAttribute('id')))
        ...
        medicine_name_error.innerHTML = "Medicament inexistant!";
        medicine_name.focus();
      }
        ...
        quantity_error.innerHTML = "Augmenter la quantite ou supprimer la ligne!";
        quantity.focus();
      }
